const fs = require('fs');
const file = 'client/src/pages/Admin/StockHistory.jsx';
...
import { 
  ...
  FaArrowUp, FaArrowDown, FaExchangeAlt, FaPrint, FaCubes
} from "react-icons/fa";`;
content = content.replace(/import React[\s\S]*?import ConfirmModal from "\.\.\/\.\.\/components\/ConfirmModal";/, newImports);
...
const getEventBadge = (type) => {
  switch (type) {
    ...
    case 'RETURN':
      return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-amber-100 text-amber-700 text-[10px] font-black uppercase tracking-wider"><FaExchangeAlt size={8}/> Return</span>;
    case 'MANUAL_ADJUSTMENT':
      return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-purple-100 text-purple-700 text-[10px] font-black uppercase tracking-wider"><FaHistory size={8}/> Adjustment</span>;
...
`;
if (!content.includes('getEventBadge')) {
  content = content.replace(/const HISTORY_PER_PAGE = 12;/, `const HISTORY_PER_PAGE = 12;\n${badgeHelper}`);
}
...
`;
if (!content.includes('detailsDrawerOpen')) {
...
const newReturn = `
  return (
    <div className="max-w-[1600px] mx-auto p-4 sm:p-8 space-y-6 animate-in fade-in duration-700" style={{ backgroundColor: '#F8FAFC', minHeight: '100vh' }}>
      ...
      {/* Advanced Filters */}
      <div className="p-4 bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-wrap lg:flex-nowrap gap-4 items-center">
        ...
        <div className="relative min-w-[180px]">
          ...
          >
            ...
            <option value="CANCELLATION_RESTOCK">Return / Cancelled</option>
        ...
        {(search || eventType || dateFrom || dateTo) && (
          <button 
            onClick={() => {
              setSearch(""); setEventType(""); setDateFrom(""); setDateTo("");
              fetchHistory(true);
            }}
          ...
          <table className="w-full text-left border-collapse table-auto min-w-[1000px]">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="px-4 py-3 text-[10px] font-black uppercase tracking-widest text-slate-500 border-r border-slate-200">Product Name</th>
                ...
                <th className="px-4 py-3 text-[10px] font-black uppercase tracking-widest text-slate-500 border-r border-slate-200 text-center">Reference Number</th>
                ...
                <th className="px-4 py-3 text-[10px] font-black uppercase tracking-widest text-slate-500 border-r border-slate-200">Remarks</th>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 border-r border-slate-100 text-center whitespace-nowrap">
                        {entry.referenceId ? (
                          <span className="font-mono text-[10px] font-bold bg-slate-100 text-slate-600 px-2 py-1 rounded">
                            {entry.referenceId}
                          </span>
                        ) : (
                      ...
                      <td className="px-4 py-3 border-r border-slate-100 text-right whitespace-nowrap">
                        <span className={\`inline-flex items-center justify-end font-black text-sm \${isZero ? 'text-slate-400' : isPositive ? 'text-emerald-600' : 'text-rose-600'}\`}>
                          {isPositive && !isZero ? "+" : ""}{entry.quantityChange} <span className="text-[10px] ml-1 opacity-70">Units</span>
                        </span>
                      </td>
                      <td className="px-4 py-3 border-r border-slate-100 text-right whitespace-nowrap">
                        <span className="font-black text-sm text-slate-800">{entry.newStock} <span className="text-[10px] ml-1 opacity-50">Units</span></span>
                      </td>
                      <td className="px-4 py-3 border-r border-slate-100 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                      ...
                      <td className="px-4 py-3 border-r border-slate-100">
                        <p className="text-xs text-slate-600 font-medium line-clamp-2" title={entry.note}>{entry.note || "—"}</p>
                      </td>
                      <td className="px-4 py-3 text-center">
                        <div className="relative inline-block dropdown-container">
                          ...
                          {openDropdownId === entry._id && (
                            <div className="absolute right-0 top-full mt-1 w-40 bg-white rounded-xl shadow-xl border border-slate-100 py-1 z-50 animate-in zoom-in-95 duration-200">
                              <button 
...
const oldReturnRegex = /return \([\s\S]*?\);\n\};/s;
content = content.replace(oldReturnRegex, newReturn + '\n};');

fs.writeFileSync(file, content);
console.log('Successfully updated StockHistory.jsx');
